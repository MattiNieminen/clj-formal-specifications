...
(deftest action-test
  (testing "account was created"
    (is (= (do
             (execute-init account-1 (create-account))
             @account-1)
           {:balance 0})))
  (testing "account balance can be modified"
    (is (true? (available? (deposit-action @account-1 100))))
    (is (= (do
             (execute (deposit-action @account-1 100) account-1)
             @account-1)
           {:balance 100}))
    (is (= (do
             (execute (withdraw-action @account-1 75) account-1)
             @account-1)
           {:balance 25})))
  (testing "larger amount than the balance of the account can't be withdrawn"
    (is (false? (available? (withdraw-action @account-1 100))))))
