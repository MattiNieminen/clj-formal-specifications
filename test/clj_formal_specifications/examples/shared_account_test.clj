...
(deftest action-test
  (testing "person was created"
    (is (= (do
             (execute-init person-1 (create-person :mike 100))
             @person-1)
           {:id :mike :wallet 100}))
    (is (= (do
             (execute-init person-2 (create-person :bob 0))
             @person-2)
           {:id :bob :wallet 0})))
  (testing "account was created"
    (is (= (do
             (execute-init account-1 (create-account #{person-2}))
             @account-1)
           {:balance 0 :owners '(:bob)})))
  (testing "it is not possible to deposit money that the person does not have"
    (is (not (available? (deposit account-1 person-2 1)))))
  (testing "person with money can deposit to an account"
    (is (= (do
             (execute (deposit account-1 person-1 100))
             @account-1)
           {:balance 100 :owners '(:bob)}))
    (is (= @person-1 {:id :mike :wallet 0})))
  (testing "person who does not own the account can not withdraw"
    (is (false? (available? (withdraw account-1 person-1 100))))
    (is (true? (available? (withdraw account-1 person-2 100)))))
  (testing "person who owns the account can withdraw"
    (is (= (do
             (execute (withdraw account-1 person-2 100))
             @account-1)
           {:balance 0 :owners '(:bob)})))
